...
month = ['January', 'February', 'March', 'April', 'May', 'Jun', 'July', 'August', 'September', 'October', 'November',
         'December']
now = date.today()
...
def main():
    ...
    awindow =   Tk()
    awindow.configure(bg='orange')

    awindow.title("Attendance Register")
    awindow.configure(background="orange")
    awindow.state("zoomed")

    sw = ScrolledWindow(awindow)
    sw.pack()

    window = tk.Frame(sw.window, background="orange")
    dict, total = load_pickle()
    # mo = 'Jun'
    var = change(dict)

    Label(window, text="-" * 250, font=("Belwe Bd BT", 10), background="orange").grid(row=0, column=0, columnspan=32)
    tk.Button(window, text="Previous", font=("Belwe Bd BT", 10), command=lambda: change_month(-1)).grid(row=1, column=0)
    Label(window, text=mo, font=("Belwe Bd BT", 15), background="orange").grid(row=1, column=1, columnspan=30)
    tk.Button(window, text="Next", font=("Belwe Bd BT", 10), command=lambda: change_month(+1)).grid(row=1, column=31)
    Label(window, text="-" * 250, font=("Belwe Bd BT", 10), background="orange").grid(row=2, column=0, columnspan=32)

    for i in range(1, 32):
        Label(window, text=i, font=("Belwe lt BT", 10), background="orange").grid(row=3, column=i)

    j = 3

    for name in labour_names:
        Label(window, text=name, font=("Belwe lt BT", 12), background="orange").grid(row=j + 1, column=0)

        for i in range(31):
            Checkbutton(window, text='', var=var[mo][name][i], onvalue=1, offvalue=0,
                        command=lambda: print(var[mo][name][i].get())).grid(column=i + 1, row=j + 1)
        j += 1

    tk.Button(window, text="Update", font=("Belwe Bd BT", 10), command=lambda ch=var: readstatus(ch)).grid(
        row=j + 3 + 1, column=0)
